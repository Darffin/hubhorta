<?php
include_once "../fachada.php";

$id = @$_GET["id"];
$login = @$_GET["login"];
$senha = @$_GET["senha"];
$nome = @$_GET["nome"];
$dao = $factory->getUsuarioDao();
$tempUsuario = $dao->buscaPorId($id);

if (empty($login) || empty($senha) || empty($nome)){
    header("Location: /hubhorta/usuario/modifica_usuario.php?erro=nao-preenchimento&id={$id}");
    exit;
}

if(($dao->buscaPorLogin($login) != null) && ($tempUsuario->getLogin() != $login)){
    header("Location: /hubhorta/usuario/modifica_usuario.php?erro=conta-ja-existente&id={$id}");
    exit;
}

$usuario = new Usuario($id,$login,md5($senha),$nome,$tempUsuario->getPermissao());


if($tempUsuario->getPermissao()=='gerenciador'){
    $daoGerenciador = $factory->getGerenciadorDao();
    $tempGerenciador = $daoGerenciador->buscaPorEmail($tempUsuario->getLogin());
    $gerenciador = new Gerenciador($tempGerenciador->getId(),$nome, $login, md5($senha));
    $daoGerenciador->altera($gerenciador);
}

$dao->altera($usuario);

header("Location: usuarios.php");
exit;

?>
